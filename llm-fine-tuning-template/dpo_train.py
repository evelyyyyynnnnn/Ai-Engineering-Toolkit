...
from data_utils import load_dpo_dataset, create_sample_dpo_data, format_dpo_data_for_training
...
def main():
    ...
    # 格式化数据集用于DPO训练
    logger.info("格式化数据集...")
    train_dataset = format_dpo_data_for_training(train_dataset, tokenizer, config['dpo']['max_length'])
    eval_dataset = format_dpo_data_for_training(eval_dataset, tokenizer, config['dpo']['max_length'])
